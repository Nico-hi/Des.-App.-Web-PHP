...
function mostrarIncidencias(lista) {
    ...
    lista.forEach(p => {
        resultados.innerHTML += `<li>ID Inc.: ${p.id} | ${p.titulo} - 
        ${p.descripcion} | En estado ${p.estado} | Asociada a usuario: 
        ${p.usuario_id} </li>`;
    });
}
...
formInsertar.addEventListener("submit", e => {
    ...
    fetch("http://localhost:3000/incidencias/crear", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
formActualizar.addEventListener("submit", e => {
    ...
    fetch(`http://localhost:3000/incidencias/actualizar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
formEliminar.addEventListener("submit", e => {
    ...
    fetch(`http://localhost:3000/incidencias/eliminar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id })
    })
