...
document.getElementById("form-insertar").addEventListener("submit", e => {
    e.preventDefault();
    fetch("public/insertar.php", { method: "POST", body: new FormData(e.target) })
        .then(r => r.json())
        .then(lista => {
            let html = "";
...
document.getElementById("form-modificar").addEventListener("submit", e => {
    e.preventDefault();
    fetch("public/modificar.php", { method: "POST", body: new FormData(e.target) })
        .then(r => r.json())
        .then(lista => {
            let html = "";
            lista.forEach(obj => html += "<li>" + obj.mensaje + "</li>");
...
document.getElementById("form-borrar").addEventListener("submit", e => {
    e.preventDefault();
    fetch("public/borrar.php", { method: "POST", body: new FormData(e.target) })
        .then(r => r.json())
        .then(lista => {
            let html = "";
            lista.forEach(obj => html += "<li>" + obj.mensaje + "</li>");
